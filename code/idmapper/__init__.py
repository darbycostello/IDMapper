...
class IDMapper(bpy.types.Operator):
	...
	similarity 		= FloatProperty			(name="Similarity",			description="How similar adjacent faces should be to be called realated (0 = not at all, 1 completely)", default=0.9, min=0, max=1, step=1, precision=3)
	seams			= BoolProperty			(name="UV Seams",			description="Consider edge seams as boundaries", default=False)
	sharp			= BoolProperty			(name="Sharp edges",		description="Consider edge marked as sharp as boundaries", default=False)
	bevel_weight	= FloatProperty			(name="Bevel threshold",	description="Consider edges with a bevel weight above this threshold as boundaries (0 disables)", default=0, min=0, max=1)
	crease_weight	= FloatProperty			(name="Crease threshold",	description="Consider edges with a crease weight above this threshold as boundaries (0 disables)", default=0, min=0, max=1)
	area_weight		= FloatProperty			(name="Area weight",		description="Consider face area in similarity test (0=off)", default=0, min=0, max=1)
	smooth			= BoolProperty			(name="Smooth",				description="Consider faces with a different smooth setting as boundaries", default=False)
	seed			= IntProperty			(name="Seed",				description="Random seed (different values give different colors)", default=0, min=0)
	merge			= BoolProperty			(name="Merge",				description="Merge adjacent isolated faces", default=False)
	match			= BoolProperty			(name="Match",				description="Give regions with the same number of faces the same color", default=False)
	matchvar 		= IntProperty			(name="Closeness",			description="How similar the faces of regions with the same number of faces should be (0 = not at all, 7 completely)", default=1, min=0, max=7)
	selected		= BoolProperty			(name="Only selected",		description="Calculate id map only for selected faces", default=False)
	basecolor		= FloatVectorProperty	(name="Base color",			description="Color assigned to non-selected faces", subtype='COLOR', default=Vector((0,0,0)))
	...
	selectmaterial	= BoolProperty			(name="Only material id",	description="Calculate id map only for faces with a material id (disabled when no materials present on mesh)", default=False)
	selectid		= IntProperty			(name="Material id",		description="Material id (slot index)", default=0, min=0)
	usedisplaycolor	= BoolProperty			(name="Use display color",	description="Use display color of assigned material", default=False)
	...
	def execute(self, context):
		...
		# get the active vertex color layer or create one
		...
		if self.materialid:
			weightmap = {f.index: f.material_index if (not selected or f.select) else -1 for f in bm.faces}
			if self.usedisplaycolor:
				ob = context.active_object
				nmats = len(ob.material_slots)
				indexed_colors = {w:list(ob.material_slots[w].material.diffuse_color) + [1.0] if (w<nmats and w>=0)else list(self.basecolor) + [1.0] for w in set(weightmap.values()) }
			else:
				indexed_colors = {w:[random(), random(), random(), 1.0] for w in set(weightmap.values()) }  # assign a random color for each unique weight
			indexed_colors[-1] = list(self.basecolor) +  [1.0] # but the default color for non selected faces is defined by the user

		else:
			# map this only once
			similarity = (self.similarity - 0.5 ) * 2.0  # map [0,1] -> [-1,1]

			# check if we have an active bevel layer
			bevel_layer = bm.edges.layers.bevel_weight.active
			bevel_weight = self.bevel_weight if bevel_layer else 0

			# check if we have an active crease layer
			...
			faces = set(f.index for f in bm.faces if (f.select or not selected) and (f.material_index == selectid or not selectmaterial))
				...
				while len(todo):
					...
					for edge in ff.edges:
						...
						for other_face in edge.link_faces:
							if selected and not other_face.select:   # skip not selected faces if requested
								continue
							if selectmaterial and not other_face.material_index == selectid:  # skip faces with the wrong material id
								continue
							ofi = other_face.index
							if (ofi in todo) or (ofi in weightmap):  # face already visited or already on our todo list
								continue
							elif self.smooth and ff.smooth != other_face.smooth:  # faces with different shading attributes
								continue
							# two faces are considered similar if their face normals point more or less in the same direction
								...
								if aq > 1 :
									aq = 1/aq # aq always <= 1, if aq == 1 areas are identical
			...
			if selectmaterial:
				weightmap.update({f.index:-1 for f in bm.faces if f.material_index != selectid})
			...
			if self.merge:
				...
				for face in bm.faces:
					...
					for edge in face.edges:
						for other_face in edge.link_faces:
							...
							if other_face.index != face.index and weightmap[other_face.index] == weightmap[face.index]:
								connected = True
								break
						if connected:
							break
				...
				while face is not None:
					...
					while len(todo):
						...
						for edge in ff.edges:
							for other_face in edge.link_faces:
								if (face != other_face.index) and (other_face.index in isolated_faces) and (other_face.index not in todo):
			...
			if self.match:
				...
				if self.matchvar > 0:
					...
					setvars = {}
					for w,fs in face_sets.items():
						setvar = round(variance[w] / maxvar, self.matchvar)
						setvars[w] = setvar
		...
		for f,weight in weightmap.items():
			...
			for loop in bm.faces[f].loops:
				vertex_colors[loop.index].color = color.copy()  # colors are wrapped so make sure each gets its unique copy
	...
	def draw(self, context):
		...
		layout = self.layout
		layout.label('Boundaries')  # should we hide or disable this options if the appropriate layer is not present?
		row = layout.row()
		row.prop(self,'materialid')
		row.active = nmats > 0
		if not self.materialid:
		...
		if not self.materialid:
			...
			row.active = nmats > 0
		if not self.materialid or not self.usedisplaycolor:
			layout.label('Options')
		if not self.materialid:
			row = layout.row()
			row.prop(self,'similarity')
			row.prop(self,'area_weight')
		if not (self.materialid and self.usedisplaycolor):
...
class VertexColorMerge(bpy.types.Operator):
	...
	operation	= EnumProperty (name="Mode"  , description="Merge mode"  , items=[('Add','Add','Add'),('Multiply','Multiply','Multiply')])
	...
	def poll(self, context):
		...
		p = (context.mode == 'PAINT_VERTEX' and
			isinstance(context.scene.objects.active, bpy.types.Object) and
			isinstance(context.scene.objects.active.data, bpy.types.Mesh) and
			len(context.scene.objects.active.data.vertex_colors)
			)
		return p
	...
	def draw(self, context):
		...
		layout.prop_search(self,'vcol1',mesh,'vertex_colors')
		layout.prop_search(self,'vcol2',mesh,'vertex_colors')
	...
	def execute(self, context):
		...
		bpy.ops.mesh.vertex_color_add()
		mesh.vertex_colors.active.name = 'Col.Merged'
		vertex_colors = mesh.vertex_colors.active.data

		vcol1 = mesh.vertex_colors[self.vcol1].data if self.vcol1 in mesh.vertex_colors else None
		vcol2 = mesh.vertex_colors[self.vcol2].data if self.vcol2 in mesh.vertex_colors else None

		default = Color((0,0,0,1))
		if self.operation == 'Add':
			for loop in mesh.loops:
				vi = loop.vertex_index
				vertex_colors[loop.index].color[:3] = (default.copy() if vcol1 is None else vcol1[loop.index].color) + (default if vcol2 is None else vcol2[loop.index].color)
		elif self.operation == 'Multiply':
			for loop in mesh.loops:
				vi = loop.vertex_index
				c1 = default if vcol1 is None else vcol1[loop.index].color  # no need to copt default here because we always create a new color to assign
				c2 = default if vcol2 is None else vcol2[loop.index].color
				vertex_colors[loop.index].color[:3] = Color((c1.r * c2.r, c1.g * c2.g, c1.b * c2.b ))  # Color objects cannot be multiplied
...
class VertexColorFromSelected(bpy.types.Operator):
	...
	def execute(self, context):
		...
		indices = set()
		indices.update(loop for f in mesh.polygons for loop in range(f.loop_start, f.loop_start + f.loop_total) if f.select)
		for i in indices:
			vertex_colors[i].color = color.copy()
...
def copy_vertex_colors(me_from, vc_from, me_to, vc_to, offset):
	...
	for face in bm_to.faces:
		...
		if location is None:
			location, normal, face_index, distance = locationb, normalb, face_indexb, distanceb
		...
		for loop in face.loops:
			vc_to[loop.index].color = color.copy()
...
class VertexColorCopy(bpy.types.Operator):
	...
	mesh_lo_name = StringProperty(name="Source mesh",	description="Source mesh to copy vertex colors from")
	offset		 = FloatProperty(name="Offset",			description="Offset for ray cast", default=0.001, min=0, max=1)
	...
	def execute(self, context):
		...
		scene = context.scene
		self.ob = context.active_object
		mesh_hi = context.scene.objects.active.data
		vertex_colors_hi = mesh_hi.vertex_colors.active.data

		if self.mesh_lo_name != '':
			mesh_lo = bpy.data.meshes[self.mesh_lo_name]
			try:
				vertex_colors_lo = mesh_lo.vertex_colors.active.data
				copy_vertex_colors(mesh_lo, vertex_colors_lo, mesh_hi, vertex_colors_hi, self.offset)
			except e:
				print(e)
...
def pick_color(context, event):
	...
	if obj.type == 'MESH':
		...
		if hit is not None:
			vertex_colors = obj.data.vertex_colors.active.data  # shoudl we check for None?
			loop_start = obj.data.polygons[face_index].loop_start
			return vertex_colors[loop_start].color[:], face_index  # we are not actually interested in the loop only its index so we use it directly to index the vertex color layer
																		# also, we copy the color because colors are wrapped values!
	buf = bgl.Buffer(bgl.GL_FLOAT, [1, 3]) # Note that if we define this buffer on a global level it gets corrupted
	bgl.glReadPixels(event.mouse_x,event.mouse_y, 1,1 , bgl.GL_RGB, bgl.GL_FLOAT, buf)
	rgb = buf[0]
	return (rgb[0],rgb[1],rgb[2]),None

def set_color(bm, context, event, color, restrict, restriction_color):
	"""set vertex colors of face under mouse pointer to color. If restrict is true only applly a new color if the original color of the face is equal to restriction_color"""
	...
	rv3d = context.region_data
	coord = event.mouse_region_x, event.mouse_region_y
	radius = context.tool_settings.unified_paint_settings.size # context.tool_settings.vertex_paint.brush.size
	radius *= radius
			...
			while len(faces):
				...
				for edge in face.edges:
					for f in edge.link_faces:  # check neighboring faces
						if not f.tag:  # exclude center face and any faces already visited
							for v in f.verts:
								if v in face.verts :  # we are only interested in verts not shared with the central face
									continue
								world_co = obj.matrix_world * v.co
								screen_co = view3d_utils.location_3d_to_region_2d(region, rv3d, world_co, default=None)
								if screen_co is None:
									break
								x = screen_co[0] - coord[0]
								y = screen_co[1] - coord[1]
								if x*x + y*y < radius:  # vert in range of cursor radius
									view_vector = view3d_utils.region_2d_to_vector_3d(region, rv3d, screen_co)
									ray_origin = view3d_utils.region_2d_to_origin_3d(region, rv3d, screen_co)
									ray_target = ray_origin + view_vector
									ray_origin_obj = m_inv * ray_origin
									ray_target_obj = m_inv * ray_target
									ray_direction_obj = ray_target_obj - ray_origin_obj
									success, location, normal, face_index = obj.ray_cast(ray_origin_obj, ray_direction_obj)
									if success and (face_index == f.index):
										faces.add(f)	 # just one qualifying vertex is enough to include the face
										break
									elif success:  # ray cast may assign a random face if they share a vertex
										fhits = bm.faces[face_index]
										fndone = False
										for vn in fhits.verts: # we check if one of the vertices of the face is indeed close to the location of the vert we checked
										...
										if fndone:
											break
						f.tag = True  # even if we didn add is this face we mark it as seen
			for f in faces_to_color:
				if not restrict or vertex_colors[f.loops[0].index].color == restriction_color:
					for loop in f.loops:
						vertex_colors[loop.index].color[:3] = color.copy() # note that color should be a unique vector (not wrapped)
...
def flatten(bm, vertex_colors):
	...
	for f in bm.faces:
		color = vertex_colors[f.loops[0].index].color
		for loop in f.loops:
			vertex_colors[loop.index].color = color.copy() # note that color should be a unique vector (not wrapped)
...
def set_region(bm, vertex_colors, match_color, face_index, color):
	...
	for f in bm.faces:
		f.tag = False
	faces = set()
	faces.add(bm.faces[face_index])  # first one we add automatically matches the match_color
	while len(faces):
		face = faces.pop()
		face.tag = True
		for loop in face.loops:
			vertex_colors[loop.index].color[:3] = color.copy()  # we are not actually interested in the loop only its index so we use it directly to index the vertex color layer
		for edge in face.edges:
			for f in edge.link_faces:
				if vertex_colors[f.loops[0].index].color[:3] == match_color[:3]:  # assume all loops of a face have the same color
					if not f.tag:
						faces.add(f)

def resize_up(bm, vertex_colors, matchcolor, face_index, restrict_color, respect_smooth, respect_seam):
	...
	while len(faces):
		...
		for edge in face.edges:
			for f in edge.link_faces:
				if vertex_colors[f.loops[0].index].color == matchcolor:  # assume all loops of a face have the same color
					if not f.tag:
						faces.add(f)
					...
					for fn in edge.link_faces:
						if not fn.tag:  # exclude self
							for loop in fn.loops:
								vertex_colors[loop.index].color = matchcolor.copy()
	else:
		for f in bm.faces:
			if f.tag:
				for edge in f.edges:
					...
					for fn in edge.link_faces:
						if (not fn.tag):
							if (vertex_colors[fn.loops[0].index].color == restrict_color):  # exclude self and faces that don't have the brush color
								for loop in fn.loops:
									vertex_colors[loop.index].color = matchcolor.copy()

def resize_down(bm, vertex_colors, matchcolor, face_index, restrict_color, respect_smooth, respect_seam):
		...
		for f in bm.faces:
			if f.tag:
				...
				for edge in f.edges:
					...
					for fn in edge.link_faces:
						if not fn.tag:  # exclude self
							color = vertex_colors[fn.loops[0].index].color
							if color != matchcolor:
								for loop in f.loops:
									vertex_colors[loop.index].color = color.copy()
								done = True
								break
					if done:
						break
...
def smooth(bm, vertex_colors, matchcolor, face_index, restrict_color, respect_smooth, respect_seam):
	# first find all faces in the color connected region (tag them)
	for f in bm.faces:
		f.tag = False
	faces = set()
	...
	while len(faces):
		...
		for edge in face.edges:
			for f in edge.link_faces:
				if vertex_colors[f.loops[0].index].color[:3] == matchcolor[:3]:  # assume all loops of a face have the same color
					if not f.tag:
						faces.add(f)
	change = {}
	nonmatch = Color()
	for face in bm.faces:
		...
		fc = vertex_colors[face.loops[0].index].color
		if face.tag:  # this face is part of the colored area, we are going to see if we will erode it (== five it a different color)
			for edge in face.edges:
				if respect_smooth and not edge.smooth:
					continue
				if respect_seam and edge.seam:
				...
				for fn in edge.link_faces:
					faces += 1
					nfc = vertex_colors[fn.loops[0].index].color
					if nfc[:3] == matchcolor[:3]:
						matches +=1
					else:
						non_matches += 1
						nonmatch = nfc
			if matches == 1:  # it only has 1 matching neighbor
				change[face] = restrict_color if restrict_color else nonmatch
		elif (restrict_color is None) or (fc[:3] == restrict_color[:3]):  # this face is part of the restricted color or we consider any adjacent face if there is no color restriction
			tag = False
			...
			if tag:  # we are next to the colored area
				if matches > 1:
					change[face] = matchcolor
	for face, color in change.items():
		for loop in face.loops:
			vertex_colors[loop.index].color[:3] = color.copy()[:3]
...
def recolor(vertex_colors, match_color, color):
	...
	for dataelement in vertex_colors:
		if dataelement.color[:3] == match_color[:3]:
			dataelement.color[:3] = color[:3]
...
def cursor_handler(self, context, shortcut):
	...
	r = context.tool_settings.unified_paint_settings.size  # context.tool_settings.vertex_paint.brush.size

	if not shortcut:  # if invoked from shortcut the vertex paint cursor stays on screen so we don't have to draw the circle ourselves
		bgl.glBegin(bgl.GL_LINE_STRIP)
		for x, y in self.circle:
			bgl.glVertex2i(self.mouse_pos[0]+int(x*r),self.mouse_pos[1]+int(y*r))
		bgl.glEnd()

	# 50% alpha, 1 pixel bracketing lines
	bgl.glBegin(bgl.GL_LINES)
	for x1,y1,x2,y2 in self.bracket:
		bgl.glVertex2i(self.mouse_pos[0]+int(x1*r),self.mouse_pos[1]+int(y1*r))
		bgl.glVertex2i(self.mouse_pos[0]+int(x2*r),self.mouse_pos[1]+int(y2*r))
	bgl.glEnd()
...
class VertexColorFacePaint(bpy.types.Operator):
	...
	def modal(self, context, event):
		...
		helptext = { 	None: 			"Click to paint, S: pick color, K: fill, H: toggle help, ESC/right mouse to exit",
		...
		if event.type in {'WHEELUPMOUSE', 'WHEELDOWNMOUSE', 'MIDDLEMOUSE'} and not event.alt and not self.mode in ('PAINT', 'CURSORRESIZE'):
			# allow navigation but we steal middlemouse actions when modified with alt or when in cursor resize mode
			return {'PASS_THROUGH'}
		elif event.type in {'NUMPAD_PERIOD','TAB', 'HOME', 'NUMPAD_2', 'NUMPAD_4', 'NUMPAD_6', 'NUMPAD_8', 'NUMPAD_1', 'NUMPAD_3', 'NUMPAD_7', 'NUMPAD_9', 'NUMPAD_5'}:
			...
			return {'PASS_THROUGH'}
		elif event.type in {'RIGHTMOUSE', 'ESC'} and not event.shift:  # we allow the customary escape and right mouselick to end the modal operation but steal shift-rightclick
			self.bm.free()
		...
		try:  # just to make sure we can free resources, reset handler etc. we catch *anything* unexpected
			numkeys = {'ONE':1,'TWO':2,'THREE':3,'FOUR':4,'FIVE':5,'SIX':6,'SEVEN':7,'EIGHT':8,'NINE':9}
			if self.mode is None:
				if event.type == 'H'and event.value == 'RELEASE':
					...
					color, face_index = pick_color(context, event)
					...
					matchcolor, face_index = pick_color(context, event)
					if face_index is not None:
						self.set_undo()
						if event.alt:
							recolor(self.vertex_colors, matchcolor, context.tool_settings.vertex_paint.brush.color)
						else:
							set_region(self.bm, self.vertex_colors, matchcolor, face_index, context.tool_settings.vertex_paint.brush.color)

				elif event.type == 'W' and event.value == 'RELEASE':
					matchcolor, face_index = pick_color(context, event)
					if face_index is not None:
						self.set_undo()
						smooth(self.bm, self.vertex_colors, matchcolor, face_index, context.tool_settings.vertex_paint.brush.color if event.alt else None, event.ctrl, event.ctrl)
					...
					self.mode = 'PAINT'
					self.set_undo()
					self.original_color, face_index = pick_color(context, event)  # this might be set to None if the cursor is not on the mesh
					set_color(self.bm, context, event, context.tool_settings.vertex_paint.brush.color, event.alt, self.original_color)
					self._set_cursor(context, self.mode)
					...
					color = context.tool_settings.vertex_paint.brush.color
					indices = set()
					indices.update(loop for f in self.mesh.polygons for loop in range(f.loop_start, f.loop_start + f.loop_total) if f.select)
					for i in indices:
						self.vertex_colors[i].color = color.copy()
				...
				elif event.type == 'NUMPAD_PLUS' and event.value == 'RELEASE':  # note that we cannot check on event.ascii == '+' because ascii values are only set on press!
					matchcolor, face_index = pick_color(context, event)
					if face_index is not None:
						self.set_undo()
						resize_up(self.bm, self.vertex_colors, matchcolor, face_index, context.tool_settings.vertex_paint.brush.color if event.alt else None, event.ctrl, event.ctrl)
				elif event.type == 'NUMPAD_MINUS' and event.value == 'RELEASE':
					matchcolor, face_index = pick_color(context, event)
					if face_index is not None:
						self.set_undo()
						resize_down(self.bm, self.vertex_colors, matchcolor, face_index, context.tool_settings.vertex_paint.brush.color if event.alt else None, event.ctrl, event.ctrl)
				elif event.type == 'NUMPAD_SLASH' and event.value == 'RELEASE':
					self.set_undo()
					flatten(self.bm, self.vertex_colors)

			elif self.mode is 'PICK':  # S was pressed
				...
				if event.type == 'LEFTMOUSE' and event.value == 'RELEASE':
					self.mode = None
					self._set_cursor(context, self.mode)
				elif event.type == 'MOUSEMOVE' :  # and event.value == 'PRESS':  # moving while pressed/non pressed
					set_color(self.bm, context, event, context.tool_settings.vertex_paint.brush.color, event.alt, self.original_color)
	...
	def set_undo(self):
		UNDODEPTH = bpy.context.user_preferences.addons[__name__].preferences.undodepth
	...
	def invoke(self, context, event):
		...
		self.mouse_pos = (event.mouse_region_x, event.mouse_region_y)  # need this for our cursor draw handler
		...
		if context.space_data.type == 'VIEW_3D':
			...
			obj = context.active_object
			self.mesh = obj.data
			self.vertex_colors = obj.data.vertex_colors.active.data  # shoudl we check for None?
			...
			self.bracket = [(-1,-0.5,-1,0.5),(1,-0.5,1,0.5),(-0.5,1,0.5,1),(-0.5,-1,0.5,-1)]

			if context.area.type == 'VIEW_3D' :  # only add handler if not invoked via shortcut becuase then the regular circle is still there
				self._handle = bpy.types.SpaceView3D.draw_handler_add(cursor_handler, args, 'WINDOW', 'POST_PIXEL')
			else:
				self._handle = None
...
class VGroupsToVertexColor(bpy.types.Operator):
	...
	cutoff_weight	= FloatProperty			(name="Weight threshold",	description="Minimum weight considered for vertex group membership", default=0, min=0, max=1)
	basecolor		= FloatVectorProperty	(name="Base color",			description="Color assigned to faces not in a vertex group", subtype='COLOR', default=Vector((0,0,0)))
	replace			= BoolProperty			(name="Replace",			description="Assign Base Color if vertex not a member of a vertex group", default=False)
	fullface		= BoolProperty			(name="Full face",			description="Give faces uniform colors", default=True)
	fullfaceall		= BoolProperty			(name="All",				description="Only faces with all verts selected", default=True)
	seed			= IntProperty			(name="Seed",				description="Random seed (different values give different colors)", default=0, min=0)
	...
	def poll(self, context):
		...
		p = (context.mode == 'PAINT_VERTEX' and
			isinstance(context.scene.objects.active, bpy.types.Object) and
			isinstance(context.scene.objects.active.data, bpy.types.Mesh) and
			len(context.scene.objects.active.vertex_groups) > 0 )
		return p
	...
	def execute(self, context):
		...
		if self.replace:
			for loop in mesh.loops:
				vertex_colors[loop.index].color = self.basecolor.copy()
		...
		for vertex_group in self.ob.vertex_groups:
			...
			if self.fullface:
				for p in mesh.polygons:
					weight = None
					if self.fullfaceall:
						for v in p.vertices: # check if all vertices are a member of a vertex group
							try:
						...
						for v in p.vertices: # check if at least one vertex is a member of a vertex group
							try:
								w = vertex_group.weight(v)
								if w >= t:
									weight = w
					...
					if weight:
						for loop in p.loop_indices:
							vertex_colors[loop].color = color.copy()
			else:
				for loop in mesh.loops:
					try:
...
class NormalizeVertexColors(bpy.types.Operator):
	...
	def execute(self, context):
		...
		# create a normlized set
		normalized_colors = color_set(len(unique_colors))

		# remap the colors
		for p in mesh.polygons:
			for i in range(p.loop_start, p.loop_start+p.loop_total):
				match = np.where(np.all(np.isclose(unique_colors,loopcolors[i], atol=0.0001), axis=1))[0]  # yes this returns a list of arrays, the first one is the one we need
				if len(match): 
...
def tri(put, v0, v1, v2):
	def min3(a,b,c): return min(a, min(b,c))
	def max3(a,b,c): return max(a, max(b,c))
	def orient2d(a,b, x,y): return (b[0]-a[0])*(y-a[1]) - (b[1]-a[1])*(x-a[0])
...
def draw_tris_np(pixels, size, color, tris, step):
	...
	xy[:,:,1],xy[:,:,0] = np.meshgrid(np.arange(size, dtype=np.int32), np.arange(size, dtype=np.int32))
	...
	for i in range(N):
		...
		w0 = tri0m1[i,0] * (xyv[:,:,1] - tri[1,1]) - (tri0m1[i,1])*(xyv[:,:,0]- tri[1,0])
		w1 = tri2m0[i,0] * (xyv[:,:,1] - tri[0,1]) - (tri2m0[i,1])*(xyv[:,:,0]- tri[0,0])
		w2 = tri1m2[i,0] * (xyv[:,:,1] - tri[2,1]) - (tri1m2[i,1])*(xyv[:,:,0]- tri[2,0])
...
class VertexColorToImageMap(bpy.types.Operator):
	...
	bl_options = {'REGISTER', 'UNDO'}
	bl_description = "Create an image map for the active vertex color layer"

	size = EnumProperty(name="Size",description="square size of imagemap",
						items= [('1024','1024x1024','a 1K map'),
								('2048','2048x2048','a 2K map'),
								('4096','4096x4096','a 4K map')])
	bleed = BoolProperty(name="Add margin", description="add extra pixel around baked colors", default = True)
	...
	def poll(self, context):
		...
		Only visible in vertex paint mode if the active object is a mesh.
		"""
		p = (context.mode == 'PAINT_VERTEX' and  # actually other modes would be fine
			isinstance(context.scene.objects.active, bpy.types.Object) and
			isinstance(context.scene.objects.active.data, bpy.types.Mesh))
		return p

	# default layout for any options present for now
	#def draw(self, context):
	#	pass # no options yet
	...
	def execute(self, context):
		...
		if mesh.uv_layers.active is None:
			...
			# unwrap the mesh
			bpy.ops.object.mode_set(mode='EDIT')
			bpy.ops.mesh.select_all(action='SELECT')  # make sure all verts are selected to get a complete map
		...
		pixels.shape = -1
		img.pixels[:]=pixels
		self.report({'INFO'}, "%d tris on a %s grid took %.1fs" % (len(tris), self.size, time() - start))
		img.update()
...
class VertexColorsToMaterials(bpy.types.Operator):
	...
	tolerance = FloatProperty(name="Tolerance", description="Per color channel tolerance in matching colors", min=0.0, max=1.0, step=1, default=0.01)
	match = BoolProperty(name="Match Color List", description="Try to use names from color list for material names", default=True)
	...
	def poll(self, context):
		...
		p = (context.mode == 'PAINT_VERTEX' and
			isinstance(context.scene.objects.active, bpy.types.Object) and
			isinstance(context.scene.objects.active.data, bpy.types.Mesh) and
			context.scene.render.engine in {'CYCLES', 'BLENDER_RENDER'})
		return p
	...
	def execute(self, context):
		...
		for p in mesh.polygons:
			for loop in p.loop_indices:
				color = vertex_colors[loop].color[:]
			...
			if color not in materialmap:
				...
				if context.scene.render.engine == 'BLENDER_RENDER':
					mat.node_tree.nodes["Material"].material = mat  # strange circular dependance in Blender internal
			else:
...
class IDMapPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		row = layout.row()
		col = row.column()
		col.template_list("ColorMap_UL", "", context.scene, "colormap", context.scene, "active_colormap", rows=5)
		brow = col.row()
		brow.operator("scene.import_id_map", text="Load", icon='FILESEL')
		save = "Save"
		if context.scene.colormap_changed:
			save += " *"
		brow.operator("scene.export_id_map", text=save, icon='SAVE_AS')
		brow.operator("scene.colormap_initialize", text="Init", icon='NEWFOLDER')
		col = row.column(align=True)
		col.operator("scene.colormap_add", icon='ZOOMIN', text="")
		col.operator("scene.colormap_remove", icon='ZOOMOUT', text="")
...
class IDMapOpsPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		if 'icons' in preview_collections:
			col.operator(IDMapper.bl_idname,icon_value=preview_collections['icons']['idmapper_icon'].icon_id)
			col.operator(VertexColorMerge.bl_idname,icon_value=preview_collections['icons']['vcolmerge_icon'].icon_id)
			col.operator(VertexColorFromSelected.bl_idname,icon_value=preview_collections['icons']['selected2vcol_icon'].icon_id)
			col.operator(VertexColorCopy.bl_idname,icon_value=preview_collections['icons']['vcolcopy_icon'].icon_id)
			col.operator(VertexColorFacePaint.bl_idname,icon_value=preview_collections['icons']['facepaint_icon'].icon_id)
			col.operator(VGroupsToVertexColor.bl_idname,icon_value=preview_collections['icons']['vgroup2vcol_icon'].icon_id)
			col.operator(VertexColorsToMaterials.bl_idname, icon_value=preview_collections['icons']['vcol2mat_icon'].icon_id)
			col.operator(VertexColorToImageMap.bl_idname, icon_value=preview_collections['icons']['vcolbake_icon'].icon_id)
			col.operator(NormalizeVertexColors.bl_idname, icon_value=preview_collections['icons']['vcolnormalize_icon'].icon_id)
		else:
...
def make_unique(self, context):
	...
	if depth or context.user_preferences.addons[__name__].preferences.allow_duplicate_names:
		return
	depth += 1
...
def index_changed(self, context):
	if context.scene.active_colormap < len(context.scene.colormap):
		context.tool_settings.vertex_paint.brush.color = tuple(context.scene.colormap[context.scene.active_colormap].color[:3])

class ColorMap(bpy.types.PropertyGroup):
	color = bpy.props.FloatVectorProperty(name="Color", 
		default=(0.0, 0.0, 0.0),
		subtype='COLOR_GAMMA', size=3, min=0, max=1, update=idcl_changed, get=None, set=None)
	name = bpy.props.StringProperty(name="Name", default="Material", update=make_unique)

class ColorMap_UL(bpy.types.UIList):  # change to UI_UL_list later for sorting and filtering
	def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
		colormap = item
		if self.layout_type in {'DEFAULT', 'COMPACT', 'GRID'}:  # we basically ignore the distinction
			row = layout.row()
			row.alignment = 'LEFT'
			rowi = row.row()
...
class SceneColormapMoveUp(bpy.types.Operator):
	...
	@classmethod
	def poll(self, context):
		return context.scene.active_colormap > 0  # implies we also have at least 2 items
	...
	def execute(self, context):
		...
		a.color, b.color = tuple(b.color), tuple(a.color) # FloatArray is wrapped so we need copy
...
class SceneColormapInitialize(bpy.types.Operator):
	...
	tolerance = FloatProperty(name="Tolerance", description="Per color channel tolerance in matching colors", min=0.0, max=1.0, step=1, default=0.01)
	noduplicates = BoolProperty(name="No duplicates", description="do not create new items if existing item matches color", default=True)
...
class ExportIDMap(bpy.types.Operator, ExportHelper):
	...
	def execute(self, context):
		...
		with open(filepath, 'w', newline='') as csvfile:
			...
			for item in context.scene.colormap:
				writer.writerow([item.name] + [format(c,".4f") for c in item.color])  # should apply %.4f
...
class ImportIDMap(bpy.types.Operator, ImportHelper):
	...
	def execute(self, context):
		paths = [join(self.directory, name.name)
...
def load_icons():
	...
	try: # if anything goes wrong, for example because we are not running 2.75+ we just ignore it
		...
		pcoll.load("facepaint_icon"		, os.path.join(my_icons_dir, "Facepaint.png"), 'IMAGE')
		pcoll.load("selected2vcol_icon"	, os.path.join(my_icons_dir, "Selected2Vcol.png"), 'IMAGE')
		pcoll.load("vcolcopy_icon"		, os.path.join(my_icons_dir, "Vcolcopy.png"), 'IMAGE')
		pcoll.load("vcolmerge_icon"		, os.path.join(my_icons_dir, "Vcolmerge.png"), 'IMAGE')
		pcoll.load("vgroup2vcol_icon"	, os.path.join(my_icons_dir, "Vgroup2Vcol.png"), 'IMAGE')
		pcoll.load("vcolbake_icon"		, os.path.join(my_icons_dir, "Vcolbake.png"), 'IMAGE')
		pcoll.load("vcol2mat_icon"		, os.path.join(my_icons_dir, "Vcoltomaterials.png"), 'IMAGE')
		pcoll.load("vcolnormalize_icon"	, os.path.join(my_icons_dir, "Vcolnormalize.png"), 'IMAGE')
...
def menu_func_vcol(self, context):
	...
	if 'icons' in preview_collections:
		self.layout.operator(IDMapper.bl_idname,icon_value=preview_collections['icons']['idmapper_icon'].icon_id)
		self.layout.operator(VertexColorMerge.bl_idname,icon_value=preview_collections['icons']['vcolmerge_icon'].icon_id)
		self.layout.operator(VertexColorFromSelected.bl_idname,icon_value=preview_collections['icons']['selected2vcol_icon'].icon_id)
		self.layout.operator(VertexColorCopy.bl_idname,icon_value=preview_collections['icons']['vcolcopy_icon'].icon_id)
		self.layout.operator(VertexColorFacePaint.bl_idname,icon_value=preview_collections['icons']['facepaint_icon'].icon_id)
		self.layout.operator(VGroupsToVertexColor.bl_idname,icon_value=preview_collections['icons']['vgroup2vcol_icon'].icon_id)
		self.layout.operator(VertexColorsToMaterials.bl_idname, icon_value=preview_collections['icons']['vcol2mat_icon'].icon_id)
		self.layout.operator(VertexColorToImageMap.bl_idname, icon_value=preview_collections['icons']['vcolbake_icon'].icon_id)
	else:
...
def register():
	...
	bpy.types.Scene.active_colormap = bpy.props.IntProperty(name = "Active", default = 0, update=index_changed)
	bpy.types.Scene.colormap_changed = bpy.props.BoolProperty(name = "Color List Changed", default = False)
	...
	kmi = km.keymap_items.new(VertexColorFacePaint.bl_idname, 'P', 'RELEASE', ctrl=False, shift=False, head=True)
